...
# Burrowed from https://lab.whitequark.org/notes/2016-10-18/implementing-an-uart-in-verilog-and-migen/
...
class UART(Module):
    def __init__(self, serial, clk_freq, baud_rate):
        self.rx_data = Signal(8)  # Received Data output
        self.rx_ready = Signal()  # this become 1 when there is valid byte in rx_data
        self.rx_ack = Signal()    # Signals received byte has read
        self.rx_error = Signal()
        self.canreceive = Signal() #Signals that UART is not receiving or transmitting. Note this make UART non-Duplex
        
        self.tx_data = Signal(8)  # Data to be transmitted placed here
        self.tx_ready = Signal()  # Pulse this to start transmission
        self.tx_ack = Signal()    # This is 1 if ready to receive new byte to tx. i.e. transmitter is not busy

        divisor = _divisor(freq_in=clk_freq, freq_out=baud_rate, max_ppm=50000)
        
        ###
        tx_notbusy = Signal() #Added for Flow Control
        rx_notbusy = Signal() #Added for Flow Control
        
        rx_counter = Signal(max=divisor)
        self.rx_strobe = rx_strobe = Signal()
        self.comb += rx_strobe.eq(rx_counter == 0)
        self.comb += self.canreceive.eq(tx_notbusy & rx_notbusy)
        ...
        self.rx_fsm.act("IDLE",
            rx_notbusy.eq(1), # Flow Control. Signals ready to receive          
            If(~serial.rx,
        ...
        self.rx_fsm.act("ERROR",
            self.rx_error.eq(1),
            NextState("IDLE")) #Added to reset the interface back in case of an issue
        ...
        self.tx_fsm.act("IDLE",
            tx_notbusy.eq(1), #Added for flow Control. Simplex mode            
            self.tx_ack.eq(1),
            If(self.tx_ready,
